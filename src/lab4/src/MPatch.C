...
// 1-D Lagrange basis weights on the integer knot grid 0..ordn-1,
// evaluated at normalized position cx. Identical in value to the
// Neville-scheme weights used by polin3 (up to rounding).
static inline void interp_w1d(double cx, int ordn, double *l)
{
    ...
    for (int j = 0; j < NN; j++)
    {
        ...
        while (notfind && Bp)
        {
            ...
            if (flag)
            {
                ...
                if (myrank == BP->rank)
                {
                    // fused interpolation: compute the geometric weights once
                    // per point and reuse them for every var in VarList.
                    // (mirrors f_global_interp geometry + polin3, with the
                    // per-var data gather following decide3d's mirror rules)
                    const int s0 = BP->shape[0], s1 = BP->shape[1];
                    double dXl = BP->X[0][1] - BP->X[0][0];
                    double dYl = BP->X[1][1] - BP->X[1][0];
                    double dZl = BP->X[2][1] - BP->X[2][0];

                    int cxI[3];
                    cxI[0] = (int)((pox[0] - BP->X[0][0]) / dXl + 0.4) + 1;
                    cxI[1] = (int)((pox[1] - BP->X[1][0]) / dYl + 0.4) + 1;
                    cxI[2] = (int)((pox[2] - BP->X[2][0]) / dZl + 0.4) + 1;

                    int cxB[3], cxT[3], cmin[3];
                    ...
                    if (Symmetry == OCTANT && fabs(BP->X[0][0]) < dXl) cmin[0] = -ordn / 2 + 1;
                    if (Symmetry == OCTANT && fabs(BP->X[1][0]) < dYl) cmin[1] = -ordn / 2 + 1;
                    if (Symmetry != NO_SYMM && fabs(BP->X[2][0]) < dZl) cmin[2] = -ordn / 2 + 1;
                    ...
                    double cx[3];
                    cx[0] = (cxB[0] > 0) ? (pox[0] - BP->X[0][cxB[0] - 1]) / dXl
                                          : (pox[0] + BP->X[0][1 - cxB[0] - 1]) / dXl;
                    cx[1] = (cxB[1] > 0) ? (pox[1] - BP->X[1][cxB[1] - 1]) / dYl
                                          : (pox[1] + BP->X[1][1 - cxB[1] - 1]) / dYl;
                    cx[2] = (cxB[2] > 0) ? (pox[2] - BP->X[2][cxB[2] - 1]) / dZl
                                          : (pox[2] + BP->X[2][1 - cxB[2] - 1]) / dZl;

                    double lx[6], ly[6], lz[6];
                    interp_w1d(cx[0], ordn, lx);
                    interp_w1d(cx[1], ordn, ly);
                    interp_w1d(cx[2], ordn, lz);

                    varl = VarList;
                    int k = 0;
                    while (varl)
                    {
                        const double *fp = BP->fgfs[varl->data->sgfn];
                        const double *soa = varl->data->SoA;
                        double acc = 0.0;
                        for (int kk = 0; kk < ordn; kk++)
                            for (int jj = 0; jj < ordn; jj++)
                                for (int ii = 0; ii < ordn; ii++)
                                {
                                    int ia = cxB[0] + ii, ja = cxB[1] + jj, ka = cxB[2] + kk;
                                    double sv = 1.0;
                                    if (ia < 1) { ia = 1 - ia; sv *= soa[0]; }
                                    if (ja < 1) { ja = 1 - ja; sv *= soa[1]; }
                                    if (ka < 1) { ka = 1 - ka; sv *= soa[2]; }
                                    acc += lx[ii] * ly[jj] * lz[kk] * sv *
                                           fp[(ka - 1) * s0 * s1 + (ja - 1) * s0 + (ia - 1)];
                                }
                        shellf[j * num_var + k] = acc;
                        varl = varl->next;
                        k++;
                    }
                    weight[j] = 1;
