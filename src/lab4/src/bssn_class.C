...
{
  // load-balance diagnosis: per-rank block counts by level
  {
    int nblk[16] = {0};
    for (int lev = 0; lev < GH->levels && lev < 16; lev++)
    {
      MyList<Patch> *Pp = GH->PatL[lev];
      while (Pp)
      {
    ...
    int all[16 * 128] = {0};
    MPI_Gather(nblk, 16, MPI_INT, all, 16, MPI_INT, 0, MPI_COMM_WORLD);
    if (myrank == 0)
    {
      int tot[16] = {0};
      cout << " BLOCKS per rank (lev0.." << GH->levels - 1 << "):" << endl;
      for (int r = 0; r < nprocs; r++)
      {
        cout << "  rank" << r << ":";
        int s = 0;
        for (int l = 0; l < GH->levels && l < 16; l++)
        {
          cout << " " << all[r * 16 + l];
          tot[l] += all[r * 16 + l];
          s += all[r * 16 + l];
      ...
      cout << " BLOCK total per rank: min=" << smin << " max=" << smax << endl;
      (void)tot;
    }
  }
