...
// memcpy-based region copy, replacing the Fortran copy() slice assignment
// (identical bytes; column-major: x fastest)
static void copy_region_memcpy(double *data_out, const double *data_in,
                               int s0o, int s1o, int s2o, int s0i, int s1i, int s2i,
                               int illo, int iuuo, int illi, int iuui,
                               int jllo, int juuo, int jlli, int juui,
                               int kllo, int kuuo, int klli, int kuui)
{
    const int nxi = iuui - illi + 1;
    const size_t nbytes = nxi * sizeof(double);
    for (int k = 0; k <= kuuo - kllo; k++)
        for (int j = 0; j <= juuo - jllo; j++)
            memcpy(data_out + ((kllo + k) - 1) * s0o * s1o + ((jllo + j) - 1) * s0o + (illo - 1),
                   data_in + ((klli + k) - 1) * s0i * s1i + ((jlli + j) - 1) * s0i + (illi - 1),
                   nbytes);
}

// index computation shared by the two f_copy call sites
static void copy_indexes(double *llbout, double *uubout, double *llbin, double *uubin,
                         double *lcopy, double *ucopy, int *ext_out, int *ext_in,
                         int *illo, int *iuuo, int *illi, int *iuui)
{
    for (int m = 0; m < 3; m++)
    {
        double ho = (uubout[m] - llbout[m]) / ext_out[m];
        double hi = (uubin[m] - llbin[m]) / ext_in[m];
        illo[m] = (int)((lcopy[m] - llbout[m]) / ho + 0.4) + 1;
        iuuo[m] = ext_out[m] - (int)((uubout[m] - ucopy[m]) / ho + 0.4);
        illi[m] = (int)((lcopy[m] - llbin[m]) / hi + 0.4) + 1;
        iuui[m] = ext_in[m] - (int)((uubin[m] - ucopy[m]) / hi + 0.4);
    }
}
...
MyList<Block> *Parallel::distribute(
    ...
    PLi = PatchLIST;
    int reacpu = 0;
    static int *blk_load = 0;
    if (!blk_load)
        blk_load = new int[cpusize]();
    while (PLi)
    {
        Patch *PP = PLi->data;
        ...
        for (int i = 0; i < nxyz[0]; i++)
            for (int j = 0; j < nxyz[1]; j++)
                for (int k = 0; k < nxyz[2]; k++)
                {
                    ...
                    // load-balanced assignment: give each new block to the
                    // rank with the fewest blocks so far (same partition as
                    // before, only the owner rank changes)
                    {
                        int best = 0;
                        for (int r = 1; r < cpusize; r++)
                            if (blk_load[r] < blk_load[best])
                                best = r;
                        ng = ng0 = new Block(dim, shape_here, bbox_here, best, ingfsi, fngfsi, PP->lev); // delete through KillBlocks
                        blk_load[best]++;
                    }
                    //	    ng->checkBlock();
                    if (BlL)
      ...
      while (varls && varld)
      {
        if (data)
        {
          if (dir == PACK)
            ...
            case 1:
            {
              int illo[3], iuuo[3], illi[3], iuui[3];
              copy_indexes(dst->data->llb, dst->data->uub, src->data->Bg->bbox, src->data->Bg->bbox + dim,
                           dst->data->llb, dst->data->uub, dst->data->shape, src->data->Bg->shape,
                           illo, iuuo, illi, iuui);
              copy_region_memcpy(data + size_out, src->data->Bg->fgfs[varls->data->sgfn],
                                 dst->data->shape[0], dst->data->shape[1], dst->data->shape[2],
                                 src->data->Bg->shape[0], src->data->Bg->shape[1], src->data->Bg->shape[2],
                                 illo[0], iuuo[0], illi[0], iuui[0],
                                 illo[1], iuuo[1], illi[1], iuui[1],
                                 illo[2], iuuo[2], illi[2], iuui[2]);
              break;
            }
            case 2:
          ...
          {
            int illo[3], iuuo[3], illi[3], iuui[3];
            copy_indexes(dst->data->Bg->bbox, dst->data->Bg->bbox + dim, dst->data->llb, dst->data->uub,
                         dst->data->llb, dst->data->uub, dst->data->Bg->shape, dst->data->shape,
                         illo, iuuo, illi, iuui);
            copy_region_memcpy(dst->data->Bg->fgfs[varld->data->sgfn], data + size_out,
                               dst->data->Bg->shape[0], dst->data->Bg->shape[1], dst->data->Bg->shape[2],
                               dst->data->shape[0], dst->data->shape[1], dst->data->shape[2],
                               illo[0], iuuo[0], illi[0], iuui[0],
                               illo[1], iuuo[1], illi[1], iuui[1],
                               illo[2], iuuo[2], illi[2], iuui[2]);
