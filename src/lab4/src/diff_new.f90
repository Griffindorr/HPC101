...
#if 0
! historical branch: original triple-loop structure
  do k=1,ex(3)-1
  do j=1,ex(2)-1
  do i=1,ex(1)-1
! x direction   
...
! boundary band uses the symmetry-mirrored gather gf()
  ...
  do j=1,ex(2)-1
  do i=1,ex(1)-1
    if(i.ge.3 .and. i.le.ex(1)-2 .and. j.ge.3 .and. j.le.ex(2)-2 .and. &
       k.ge.3 .and. k.le.ex(3)-2) cycle
   if(i+2 <= imax .and. i-2 >= imin .and. &
      j+2 <= jmax .and. j-2 >= jmin .and. &
      k+2 <= kmax .and. k-2 >= kmin) then
      fx(i,j,k)=d12dx*(gf(i-2,j,k)-EIT*gf(i-1,j,k)+EIT*gf(i+1,j,k)-gf(i+2,j,k))
      fy(i,j,k)=d12dy*(gf(i,j-2,k)-EIT*gf(i,j-1,k)+EIT*gf(i,j+1,k)-gf(i,j+2,k))
      fz(i,j,k)=d12dz*(gf(i,j,k-2)-EIT*gf(i,j,k-1)+EIT*gf(i,j,k+1)-gf(i,j,k+2))
   elseif(i+1 <= imax .and. i-1 >= imin .and. &
          j+1 <= jmax .and. j-1 >= jmin .and. &
          k+1 <= kmax .and. k-1 >= kmin) then
      fx(i,j,k)=d2dx*(-gf(i-1,j,k)+gf(i+1,j,k))
      fy(i,j,k)=d2dy*(-gf(i,j-1,k)+gf(i,j+1,k))
      fz(i,j,k)=d2dz*(-gf(i,j,k-1)+gf(i,j,k+1))
   endif
  enddo
  enddo
  enddo
...
! boundary band uses the symmetry-mirrored gather gf()
   ...
   if(i+2 <= imax .and. i-2 >= imin .and. &
      j+2 <= jmax .and. j-2 >= jmin .and. &
      k+2 <= kmax .and. k-2 >= kmin) then
   fxx(i,j,k) = Fdxdx*(-gf(i-2,j,k)+F16*gf(i-1,j,k)-F30*gf(i,j,k) &
                       -gf(i+2,j,k)+F16*gf(i+1,j,k)              )
   fyy(i,j,k) = Fdydy*(-gf(i,j-2,k)+F16*gf(i,j-1,k)-F30*gf(i,j,k) &
                       -gf(i,j+2,k)+F16*gf(i,j+1,k)              )
   fzz(i,j,k) = Fdzdz*(-gf(i,j,k-2)+F16*gf(i,j,k-1)-F30*gf(i,j,k) &
                       -gf(i,j,k+2)+F16*gf(i,j,k+1)              )
   fxy(i,j,k) = Fdxdy*(     (gf(i-2,j-2,k)-F8*gf(i-1,j-2,k)+F8*gf(i+1,j-2,k)-gf(i+2,j-2,k))  &
                       -F8 *(gf(i-2,j-1,k)-F8*gf(i-1,j-1,k)+F8*gf(i+1,j-1,k)-gf(i+2,j-1,k))  &
                       +F8 *(gf(i-2,j+1,k)-F8*gf(i-1,j+1,k)+F8*gf(i+1,j+1,k)-gf(i+2,j+1,k))  &
                       -    (gf(i-2,j+2,k)-F8*gf(i-1,j+2,k)+F8*gf(i+1,j+2,k)-gf(i+2,j+2,k)))
   fxz(i,j,k) = Fdxdz*(     (gf(i-2,j,k-2)-F8*gf(i-1,j,k-2)+F8*gf(i+1,j,k-2)-gf(i+2,j,k-2))  &
                       -F8 *(gf(i-2,j,k-1)-F8*gf(i-1,j,k-1)+F8*gf(i+1,j,k-1)-gf(i+2,j,k-1))  &
                       +F8 *(gf(i-2,j,k+1)-F8*gf(i-1,j,k+1)+F8*gf(i+1,j,k+1)-gf(i+2,j,k+1))  &
                       -    (gf(i-2,j,k+2)-F8*gf(i-1,j,k+2)+F8*gf(i+1,j,k+2)-gf(i+2,j,k+2)))
   fyz(i,j,k) = Fdydz*(     (gf(i,j-2,k-2)-F8*gf(i,j-1,k-2)+F8*gf(i,j+1,k-2)-gf(i,j+2,k-2))  &
                       -F8 *(gf(i,j-2,k-1)-F8*gf(i,j-1,k-1)+F8*gf(i,j+1,k-1)-gf(i,j+2,k-1))  &
                       +F8 *(gf(i,j-2,k+1)-F8*gf(i,j-1,k+1)+F8*gf(i,j+1,k+1)-gf(i,j+2,k+1))  &
                       -    (gf(i,j-2,k+2)-F8*gf(i,j-1,k+2)+F8*gf(i,j+1,k+2)-gf(i,j+2,k+2)))
   elseif(i+1 <= imax .and. i-1 >= imin .and. &
          j+1 <= jmax .and. j-1 >= jmin .and. &
          k+1 <= kmax .and. k-1 >= kmin) then
   fxx(i,j,k) = Sdxdx*(gf(i-1,j,k)-TWO*gf(i,j,k) &
                      +gf(i+1,j,k)              )
   fyy(i,j,k) = Sdydy*(gf(i,j-1,k)-TWO*gf(i,j,k) &
                      +gf(i,j+1,k)              )
   fzz(i,j,k) = Sdzdz*(gf(i,j,k-1)-TWO*gf(i,j,k) &
                      +gf(i,j,k+1)              )
   fxy(i,j,k) = Sdxdy*(gf(i-1,j-1,k)-gf(i+1,j-1,k)-gf(i-1,j+1,k)+gf(i+1,j+1,k))
   fxz(i,j,k) = Sdxdz*(gf(i-1,j,k-1)-gf(i+1,j,k-1)-gf(i-1,j,k+1)+gf(i+1,j,k+1))
   fyz(i,j,k) = Sdydz*(gf(i,j-1,k-1)-gf(i,j+1,k-1)-gf(i,j-1,k+1)+gf(i,j+1,k+1))
   endif
   enddo
   enddo
   enddo
