...
!~~~~~~> Other variables:
  ...
  logical, save     :: rhs_timing_init = .false.
  logical, save     :: rhs_timing_enabled = .false.
  logical, save     :: rhs_timing_rank0 = .false.
  integer, save     :: rhs_timing_interval = 50
  integer, save     :: rhs_timing_calls = 0
  real*8, save      :: rhs_timing_total = 0.d0
  real*8, save      :: rhs_timing_sanity = 0.d0
  real*8, save      :: rhs_timing_metric = 0.d0
  real*8, save      :: rhs_timing_gamma = 0.d0
  real*8, save      :: rhs_timing_ricci = 0.d0
  real*8, save      :: rhs_timing_ricci_conn = 0.d0
  real*8, save      :: rhs_timing_ricci_fd = 0.d0
  real*8, save      :: rhs_timing_ricci_formula = 0.d0
  real*8, save      :: rhs_timing_ricci_chi = 0.d0
  real*8, save      :: rhs_timing_ricci_lapse = 0.d0
  real*8, save      :: rhs_timing_base_rhs = 0.d0
  real*8, save      :: rhs_timing_lopsided = 0.d0
  real*8, save      :: rhs_timing_kodis = 0.d0
  real*8, save      :: rhs_timing_constraint = 0.d0
  real*8            :: rhs_time_start, rhs_section_start, rhs_time_now
  real*8            :: rhs_other_time
  character(len=32) :: rhs_env
  integer           :: rhs_env_status


  if(.not.rhs_timing_init) then
     call get_environment_variable("BSSN_RHS_TIMING", rhs_env, status=rhs_env_status)
     rhs_timing_enabled = rhs_env_status == 0 .and. &
          trim(rhs_env) /= "0" .and. trim(rhs_env) /= "no" .and. trim(rhs_env) /= "NO"
     call get_environment_variable("OMPI_COMM_WORLD_RANK", rhs_env, status=rhs_env_status)
     if(rhs_env_status /= 0) call get_environment_variable("PMI_RANK", rhs_env, status=rhs_env_status)
     if(rhs_env_status /= 0) call get_environment_variable("PMIX_RANK", rhs_env, status=rhs_env_status)
     rhs_timing_rank0 = rhs_env_status /= 0 .or. trim(rhs_env) == "0"
     call get_environment_variable("BSSN_RHS_TIMING_INTERVAL", rhs_env, status=rhs_env_status)
     if(rhs_env_status == 0) read(rhs_env, *, err=10) rhs_timing_interval
10   continue
     rhs_timing_interval = max(1, rhs_timing_interval)
     rhs_timing_init = .true.
  endif

  if(rhs_timing_enabled) then
     rhs_timing_calls = rhs_timing_calls + 1
     call cpu_time(rhs_time_start)
     rhs_section_start = rhs_time_start
  endif
...
!!! sanity check
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_sanity = rhs_timing_sanity + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
...
! Raise indices of \tilde A_{ij} and store in R_ij
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_metric = rhs_timing_metric + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
...
! Right hand side for Gam^i without shift terms...
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_gamma = rhs_timing_gamma + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
...
!first kind of connection stored in gij,k
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_ricci_conn = rhs_timing_ricci_conn + rhs_time_now - rhs_section_start
     rhs_timing_ricci = rhs_timing_ricci + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
...
!compute Ricci tensor for tilted metric
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_ricci_fd = rhs_timing_ricci_fd + rhs_time_now - rhs_section_start
     rhs_timing_ricci = rhs_timing_ricci + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_ricci_formula = rhs_timing_ricci_formula + rhs_time_now - rhs_section_start
     rhs_timing_ricci = rhs_timing_ricci + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
...
! Add chi part to Ricci tensor:
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_ricci_chi = rhs_timing_ricci_chi + rhs_time_now - rhs_section_start
     rhs_timing_ricci = rhs_timing_ricci + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
...
! Compute trace-free part (note: chi^-1 and chi cancel!):
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_ricci_lapse = rhs_timing_ricci_lapse + rhs_time_now - rhs_section_start
     rhs_timing_ricci = rhs_timing_ricci + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
...
!!!! gauge variable part
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_base_rhs = rhs_timing_base_rhs + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
...
!!
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_lopsided = rhs_timing_lopsided + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
...
!! bam does not apply dissipation on gauge variables
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_kodis = rhs_timing_kodis + rhs_time_now - rhs_section_start
     rhs_section_start = rhs_time_now
  endif
...
movz_Res = movz_Res - F2o3*Kz - F8*PI*sz
  ...
  if(rhs_timing_enabled) then
     call cpu_time(rhs_time_now)
     rhs_timing_constraint = rhs_timing_constraint + rhs_time_now - rhs_section_start
     rhs_timing_total = rhs_timing_total + rhs_time_now - rhs_time_start
     if(rhs_timing_rank0 .and. mod(rhs_timing_calls, rhs_timing_interval) == 0) then
        rhs_other_time = rhs_timing_total - rhs_timing_sanity - rhs_timing_metric - &
             rhs_timing_gamma - rhs_timing_ricci - rhs_timing_base_rhs - &
             rhs_timing_lopsided - rhs_timing_kodis - rhs_timing_constraint
        write(*,'(A,I8,A,F10.4,A)') "BSSN_RHS_TIMING calls=", rhs_timing_calls, &
             " total_cpu=", rhs_timing_total, "s"
        write(*,'(A,9(A,F6.2))') "BSSN_RHS_TIMING pct ", &
             "sanity=", 100.d0*rhs_timing_sanity/rhs_timing_total, &
             " metric=", 100.d0*rhs_timing_metric/rhs_timing_total, &
             " gamma=", 100.d0*rhs_timing_gamma/rhs_timing_total, &
             " ricci=", 100.d0*rhs_timing_ricci/rhs_timing_total, &
             " base=", 100.d0*rhs_timing_base_rhs/rhs_timing_total, &
             " lopsided=", 100.d0*rhs_timing_lopsided/rhs_timing_total, &
             " kodis=", 100.d0*rhs_timing_kodis/rhs_timing_total, &
             " constraint=", 100.d0*rhs_timing_constraint/rhs_timing_total, &
             " other=", 100.d0*rhs_other_time/rhs_timing_total
        write(*,'(A,5(A,F6.2))') "BSSN_RHS_TIMING ricci_pct ", &
             "conn=", 100.d0*rhs_timing_ricci_conn/rhs_timing_total, &
             " fd=", 100.d0*rhs_timing_ricci_fd/rhs_timing_total, &
             " formula=", 100.d0*rhs_timing_ricci_formula/rhs_timing_total, &
             " chi=", 100.d0*rhs_timing_ricci_chi/rhs_timing_total, &
             " lapse=", 100.d0*rhs_timing_ricci_lapse/rhs_timing_total
     endif
  endif
