...
using moe_common::reduce_add_epi32_256;
...
int g_affinity_cpus[MAX_TOP_K + 1] = {};
...
static MOE_NOINLINE float ffn_gate_up_packed(
    ...
    const int n = f_end - f_begin;

    float vg_buf[MAX_D_FF];
    float vu_buf[MAX_D_FF];

    for (int fi = 0; fi < n; fi += kGateUpRowBlock) {
        const int f = f_begin + fi;
        ...
        const int8_t* wg_group =
            w_gate + (size_t)(f / kGateUpRowBlock) * group_stride;
        const int8_t* wu_group =
            w_up + (size_t)(f / kGateUpRowBlock) * group_stride;

        for (int cb = 0; cb < col_chunks; cb++) {
            const int d = cb * kGateUpColBlock;
            const size_t block =
                (size_t)cb * kGateUpRowBlock * kGateUpColBlock;
        ...
        vu_buf[fi] = (float)(reduce_add_epi32_512(acc_u0) - correction) *
                     scale_u;
        vu_buf[fi + 1] =
    ...
    for (int fi = 0; fi < n; fi += 16) {
        __m512 vg = _mm512_loadu_ps(vg_buf + fi);
        __m512 vu = _mm512_loadu_ps(vu_buf + fi);
        ...
        _mm512_storeu_ps(h + f_begin + fi, hv);
    }
    return _mm512_reduce_max_ps(maxv);
}

static MOE_NOINLINE int32_t ffn_requantize_range(
    const float* h, int8_t* hq, int f_begin, int f_end, float s_h) {
    ...
    return _mm512_reduce_add_epi32(sumv);
}

static MOE_NOINLINE void ffn_down_packed(const int8_t* w_down, float s_down,
                                         const int8_t* hq, int d_ff,
                                         int32_t correction, float* out,
    ...
    for (int d = d_begin; d < d_end; d += kDownRowBlock) {
        ...
        for (int cb = 0; cb < col_chunks; cb++) {
            ...
            __m512i wd4 = _mm512_loadu_si512(
                (const __m512i*)(wd_group + block + 4 * kDownColBlock));
            acc4 = _mm512_dpbusd_epi32(acc4, wd4, hv8);

            __m512i wd5 = _mm512_loadu_si512(
                (const __m512i*)(wd_group + block + 5 * kDownColBlock));
            acc5 = _mm512_dpbusd_epi32(acc5, wd5, hv8);

            __m512i wd6 = _mm512_loadu_si512(
                (const __m512i*)(wd_group + block + 6 * kDownColBlock));
            acc6 = _mm512_dpbusd_epi32(acc6, wd6, hv8);

            __m512i wd7 = _mm512_loadu_si512(
                (const __m512i*)(wd_group + block + 7 * kDownColBlock));
            acc7 = _mm512_dpbusd_epi32(acc7, wd7, hv8);
        }
        ...
        out[d + 4] = (float)(reduce_add_epi32_512(acc4) - correction) * s_down;
        out[d + 5] = (float)(reduce_add_epi32_512(acc5) - correction) * s_down;
        out[d + 6] = (float)(reduce_add_epi32_512(acc6) - correction) * s_down;
        out[d + 7] = (float)(reduce_add_epi32_512(acc7) - correction) * s_down;
    }
}

static MOE_NOINLINE void expert_ffn_packed(
    ...
    float h_amax = ffn_gate_up_packed(w_gate, w_up, s_gate, s_up, xq, s_x,
                                      x_correction, h, d_model, 0, d_ff);

    int8_t hq[MAX_D_FF];
    float s_h = (h_amax > 0.0f) ? h_amax / 127.0f : 1.0f;
    int32_t sum_hq = ffn_requantize_range(h, hq, 0, d_ff, s_h);

    ffn_down_packed(w_down, s_h * s_down * out_scale, hq, d_ff,
                    sum_hq * 128, out, 0, d_model);
}
...
static void collect_affinity_cpus() {
    ...
    for (int cpu = 0; cpu < CPU_SETSIZE && g_affinity_count < MAX_TOP_K + 1; cpu++) {
...
static void expert_worker_loop(int worker_id) {
    ...
    while (true) {
        ...
        if (worker_id < g_worker_top_k) {
            ...
            expert_ffn_packed(g_w_gate_pack + (size_t)e * gate_size,
                              g_w_up_pack + (size_t)e * gate_size,
                              g_w_down_pack + (size_t)e * down_size,
                              w.s_gate[e], w.s_up[e], w.s_down[e],
                              g_worker_xq, g_worker_s_x,
                              g_worker_x_correction, g_worker_gate[worker_id],
                              g_worker_expert_out[worker_id + 1], d_model,
                              d_ff);
        }
...
static MOE_NOINLINE void process_one_token_parallel_experts(const float* xt,
    ...
    expert_ffn_packed(g_sh_gate_pack, g_sh_up_pack, g_sh_down_pack,
                      w.sh_s_gate, w.sh_s_up, w.sh_s_down, xq, s_x,
                      x_correction, 1.0f, expert_out[0], d_model, d_ff);
